...
.setup.done: 
	...
	$(LLVM_OBJ)/opt -O2 -disable-inlining -disable-loop-vectorization -disable-slp-vectorization  $(FUNCTION)/$(NAME).bc -o $(FUNCTION)/$(NAME).bc
	@touch .setup.done
...
needle-sequence: .epp-decode.done .needle-sequence.done
.needle-sequence.done: .epp-decode.done 
	@echo "NEEDLE-SEQUENCE"
	cd $(FUNCTION) && \
	export PATH=$(LLVM_OBJ):$(PATH) && \
    $(NEEDLE_OBJ)/needle -fn=$(FUNCTION) -ExtractType::sequence -seq=path-seq-0.txt $(LIBS) -u=$(HELPER_LIB) $(NAME).bc -o $(NAME)-needle-0 2>&1 > ../needle-sequence.log
