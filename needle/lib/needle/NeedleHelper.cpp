...
void static generalStats(Function& func, vector<BasicBlock*> &F, string fname) {
...
#include "llvm/IR/Instruction.def"
    ...
    Outfile << "nLiveouts " << nLiveouts << "\n";
    Outfile << "nLiveins " << nLiveins <<"\n";
...
void NeedleHelper::addUndoLog(Function *Offload) {
    ...
    CallInst::Create(Memset, Params, "")
        ->insertBefore(&*Offload->getEntryBlock().getFirstInsertionPt());
...
bool NeedleHelper::runOnModule(Module &M) {
    ...
    for (auto &F : M) {
        ...
        generalStats(F, bbs, (F.getName()+".stats.txt").str());
